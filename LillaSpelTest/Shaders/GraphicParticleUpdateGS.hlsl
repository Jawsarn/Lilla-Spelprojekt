...
void GS( point Particle input[1],  inout PointStream< Particle > ptStream )
{
	...
	//float3 vel = t*worldAcceler + input[0].InitialVelW;

	//if (input[0].Type > 0)
	//{
	//	if (t >= 0.5) //may swap this to something
	//	{
	//		for (int i = 0; i < initSpawnAmount; i++) //something
	//		{
	//			float3 random = 50*RandUnitVec3((float)i/(pos.x+pos.y/10));
	//			
	//			Particle p;
	//			p.InitialPosW = pos;
	//			p.InitialVelW = emitDirection + vel + random;
	//			p.SizeW = initialSize;
	//			p.Age = 0.0f;
